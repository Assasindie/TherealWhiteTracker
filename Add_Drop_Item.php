...
$sql = "SELECT * FROM `whitebagtypes` WHERE `WhiteBagDropLocation` = '".$_POST['DropLocation']."'";
	...
	<div id="ww">
	    <div class="container">
			<div class="row">
				...
				<font size = "15"> Select Drop from <?php echo $_SESSION['DropLocation'];?> </font></br> </br>
						<form method = "POST" action = "Add_Drop_Complete.php">
						<select name = "Drop" id = "Drop"> 
						
					<?php 
					while($result = mysqli_fetch_assoc($sql)){
					if ($drop !== $result['WhiteBagType']){	
					?> <option value = "<?php echo $result['WhiteBagType'] ?>"> <?php echo $result['WhiteBagType']?> </option>
					<?php $drop = $result['WhiteBagType']; ?>
					<?php } ?>
						<?php }?>
						</select>
					Image Link <input type = "text" name = "WhiteBagLink" id = "WhiteBagLink" required></input> </br> </br>
					Drop Date <input type = "Date" name = "Date" style="width : 200px" required >  </input> </br> </br>
